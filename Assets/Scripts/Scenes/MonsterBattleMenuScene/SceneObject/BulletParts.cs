...
public class BulletParts : MonoBehaviour {
	...
	public void DownRollStatusInfoParts(int addNumber) {
		...
		for (int i = 1; i < eventStatusInfosParts_.Count - 2; ++i) {
			eventStatusInfosParts_[i].GetStatusInfoParts().GetEventGameObject().ProcessStatePosMoveYExecute(
				0.5f
				, eventStatusInfosParts_[i].GetStatusInfoParts().transform.position.y - 1.5f
				) ;
		...
		//4番目を上に移す
		eventStatusInfosParts_[4].GetStatusInfoParts().GetEventGameObject().ProcessStatePosMoveYExecute(
			0
			, 3.5f
			);
	...
	public void UpRollStatusInfoParts(int addNumber) {
		...
		//0番目を下に移す
		eventStatusInfosParts_[0].GetStatusInfoParts().GetEventGameObject().ProcessStatePosMoveYExecute(
			0
			, 0.5f
			);
